...
@break
@case('pdf')
<div class="my-3"><a href="{{ $media->getUrl() }}" title="{{ $file->name }}" data-lity><img src="{{ $media->getUrl('thumb') }}" alt="{{ $file->name }}"><span class="mt-1 d-block small"><i class="fas fa-search-plus fa-fw mr-1"></i>@lang('Preview', [], $locale) {{ $file->name }}</span></a></div>
@break
@case('audio')
